...
let StocksController = class StocksController {
    //constructor(private data: Data){} // remove
    getAllStocks(request, response) {
        response.header("Access-Control-Allow-Origin", "*");
        response.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
        console.log("This action returns all stocks");
        return response.send(data_1.default.stocks);
    }
    getAllStocksInPortfolio(request, response) {
        response.header("Access-Control-Allow-Origin", "*");
        response.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
        console.log("This action returns all stocks portfolio");
        return response.send(data_1.default.stocksInPortfolio);
        //return "This action returns all stocks";
    }
    getAllHistory(request, response) {
        response.header("Access-Control-Allow-Origin", "*");
        response.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
        console.log("This action returns all stocks history");
        return response.send(data_1.default.stocksHistory);
        //return "This action returns all stocks";
    }
};
__decorate([
